...
if (empty($files)) {
    throw new moodle_exception('Template ZIP not found.');
}
...
foreach ($allfiles as $relpath) {
    ...
    foreach ($editable_extensions as $ext) {
...
foreach ($allfiles as $relpath) {
    if (strtolower(basename($relpath)) === 'makefile') {
        $makefile_path = $unzippath . '/' . $relpath;
        $lines = file($makefile_path);
        $modified = false;

        foreach ($lines as &$line) {
            if (preg_match('/^\s*PORT\s*=/', $line)) {
                $line = 'PORT=' . $platform_ports[$platform] . PHP_EOL;
                $modified = true;
            }
        }

        if ($modified) {
            file_put_contents($makefile_path, implode('', $lines));
        }
    }
}
